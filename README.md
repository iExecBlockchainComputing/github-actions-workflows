...
Builds, tests, analyses and publishes a Gradle-based Java project. Runs the unit tests and a SonarCloud analysis, publishes jars to a Maven repository, and hands the built jar to `docker-build` so that the artifact shipped in the image is exactly the one that was tested. The GitHub Actions counterpart of the `buildJavaProject` Jenkins pipeline.
